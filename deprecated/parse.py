...
def parse_raw(path):
    """
    Parses raw data from a WAVI EEG recording persists it to a CSV file.
    ...
    def combine_eeg_files(folder):
        ...
        print(combined_data.columns[1:21])
        # combined_data.to_csv(output_file_name)
        # change from pandas to numpy
    ...
    return [WaviDataset(*combine_eeg_files(subfolder)) for subfolder in usable_folders]
